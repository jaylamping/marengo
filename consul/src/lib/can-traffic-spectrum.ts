...
function finiteNumber(value: bigint | undefined): number | null {
  if (value == null) {
    return null;
  }
...
export function readCanLiveChip(metrics: HostMetrics | null): CanLiveChip {
  ...
  return {
    ...
    rxBytesPerSec: finiteNumber(iface?.rxBytesPerSec),
    txBytesPerSec: finiteNumber(iface?.txBytesPerSec),
    txErrorCount: finiteNumber(iface?.canTxErrorCount),
    rxErrorCount: finiteNumber(iface?.canRxErrorCount),
  };
}
...
export function appendHzSample(
  ...
  return [...previous, sample].slice(-capacity);
}
...
export function projectMicroLog(
  ...
  return frames.slice(-capacity).map((frame) => {
    const data = frame.data.replace(/\s+/g, '');
    return {
      lineNo: frame.line_no,
      offsetS: frame.offset_s ?? frame.delta_s,
      iface: frame.interface,
      canId: frame.can_id,
      dataHead: data.length > 16 ? `${data.slice(0, 16)}…` : data,
      joint: frame.joint,
...
export function buildCanTrafficSpectrum(input: BuildSpectrumInput): CanTrafficSpectrum {
  ...
  if (!summary && (summaryError || pageError)) {
    return idleSpectrum(
      live,
      nowMs,
      'unavailable',
      summaryError ?? pageError,
      previous?.rateHz ?? [],
    );
  }
  ...
  const unchanged = fingerprint != null && fingerprint === previous?.fingerprint;
