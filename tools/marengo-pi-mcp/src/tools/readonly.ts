      ...
      handler: async () => {
        const body = wrapRemote(
          ...
          [
            ...
            "echo",
            "echo '=== systemd ==='",
            "systemctl is-active marengo-can marengo-gateway marengo-auto-learn 2>/dev/null || true",
            "echo",
            "echo '=== gateway ==='",
            'curl -sf http://127.0.0.1:8080/health 2>/dev/null || echo "(gateway /health unreachable)"',
            'test -f www/index.html && echo "www/index.html ok" || echo "(missing www/index.html)"',
            "echo",
            "echo '=== auto-learn BFF ==='",
            "systemctl is-active marengo-auto-learn 2>/dev/null || echo inactive",
            'curl -sf http://127.0.0.1:8787/health 2>/dev/null || echo "(auto-learn /health unreachable — set CURSOR_API_KEY+AUTO_LEARN_TOKEN or unit not installed)"',
            "echo",
