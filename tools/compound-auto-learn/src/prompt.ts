...
export function buildAutoLearnPrompt(request: AutoLearnRequest): string {
  ...
  return [
    ...
    '',
    '## Movement brief (what this gesture is — follow this)',
    request.base.movementBrief,
    '',
    ...
    '',
    '## Base preset (shipped program / teach kind)',
    JSON.stringify(
      {
        name: request.base.name,
        description: request.base.description,
        joints: request.base.joints,
        teachKind: request.base.teachKind,
        keyframes: request.base.keyframes,
        nativeWave: request.base.nativeWave,
      },
      null,
      2,
    ),
  ...
  "source": "auto_learn"
    ...
    '- Landmark q trajectories must realize the movement brief within the stage envelope.',
    ...
    '- When operator feedback is present, prioritize it over expanding ROM and over the shipped keyframe numbers.',
