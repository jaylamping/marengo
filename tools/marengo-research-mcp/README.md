...
`GITHUB_TOKEN` is optional but helps with rate limits. The launcher (`run-mcp.ps1` / `run-mcp.sh`) pulls it from `gh auth token` when you are logged in via GitHub CLI. On Windows when `sh` is missing from Cursor’s PATH, use `node dist/launch.js` (from `launch.ts` via `just mcp-build` / `just research-mcp-setup`) as the Node entry.
