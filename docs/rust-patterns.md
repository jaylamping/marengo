...
- **Breakaway onset:** full `fc` pulse + `max(max_lead, 0.15)` only when retarget starts near home (`|q| ≤ POSITION_RETURN_DESCENT_SEED_RAD`); cut Coulomb assist when measured speed outruns `dq_ref` by more than the velocity deadband.
- **Return lag:** freeze planner only on return-to-home descent (`target ≈ 0`) while stuck lagging (`q − q_traj > POSITION_RETURN_RESYNC_RAD`, hysteresis on filtered `dq` — encoder dither cannot re-advance `q_traj`); unfreeze when synced within `POSITION_RETURN_RESYNC_RAD`; resync when planner holds at target but arm is still far.
