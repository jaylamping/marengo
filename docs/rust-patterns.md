...
- **Breakaway onset:** full `fc` pulse + `max(max_lead, 0.15)` when retarget starts near home outbound (`|q| ≤ POSITION_RETURN_DESCENT_SEED_RAD`) or on return-to-home from high `q`; cut Coulomb assist when measured speed outruns `dq_ref` by more than the velocity deadband.
- **Return lag:** freeze planner only on return-to-home descent (`target ≈ 0`) in the final band (`q ≤ POSITION_RETURN_FREEZE_Q_MAX_RAD`) while stuck lagging (`q − q_traj > POSITION_RETURN_RESYNC_RAD`, hysteresis on filtered `dq`); unfreeze when synced within `POSITION_RETURN_RESYNC_RAD`; resync when planner holds at target but arm is still far. **Stuck lead:** if `|q − q_traj| > max_lead` at rest, resync `q_traj` to `q` so the reference catches up (does not apply while moving — runaway still brakes via clamp).
...
- **Velocity cap (ADR 0010):** `marengo-config::resolve_joint_velocity_cap` merges desired cap (joint → actuator group → motor type) with URDF and bench ceilings into `JointLimitPolicy.velocity`. Berthier clamps planner `v_max` to `Supervisor::joint_velocity_cap`; Davout MIT/speed-mode filters use the same effective cap.
