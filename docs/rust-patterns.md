...
- **Return lag / ascent stall:** freeze planner on return-to-home descent (`target ≈ 0`) in the final band (`q ≤ POSITION_RETURN_FREEZE_Q_MAX_RAD`) while stuck lagging (`q − q_traj > POSITION_RETURN_RESYNC_RAD`, hysteresis on filtered `dq`). On outbound ascent stall, when `q_traj` is ahead of `q` by more than `POSITION_RETURN_RESYNC_RAD`, the arm is at rest, and the target is not settled, enter **bounded recovery**: keep the planner advancing, keep `q_des` lead-bounded, and resync `q_traj` to `q` at `max_lead`. Recovery stays armed across that resync so a true stall cannot retry forever. Sustained recovery with no measured progress (>2 s) faults the tick (disable path). Lead-follow residuals within `POSITION_RETURN_RESYNC_RAD` remain exempt. Beyond that band, recovery adds `POSITION_HOME_FINAL_PULL_THROUGH_RAD` past the outbound target within `max_lead` and arms the same fuse. **Reset the accumulator on any `dq` progress toward target** (slow knee crawl must not false-disable). Tagged `ascent_breakaway` in `planner_event`; fuse ms is on hold diag / 1 Hz hold log. Stuck premature Hold (planner at target, arm short, `|dq| < deadband`) must **not** reopen Cruise at the latched target. Prefer bounded recovery and resync. When Hold is latched but measured `q` is still short by more than `POSITION_HOME_SETTLE_RAD` (residual inside the premature band), **lead-follow finish**: keep `q_traj` at the lead-capped pose (often `target` when remaining `< max_lead`), set cruise `dq_traj` to slew, and freeze the open-loop tick so trajectory-velocity friction stays engaged. Do not dead-Hold P-creep or restart a full trapezoid (that thrashes). Bidirectional: positive targets finish from below; **home (`target≈0`) finishes from above only**. Past-home must not lead-follow back up (that oscillates with pull-through). **Stuck lead:** if `|q − q_traj| > max_lead` at rest, resync `q_traj` to `q` so the reference catches up. This does not apply while moving; runaway still brakes via clamp.
