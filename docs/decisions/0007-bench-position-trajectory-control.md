...
After **`3f66ea2`** (high-q return freeze skip, stuck-lead resync, return breakaway onset) and trajectory retune on `arm_3dof_right`:
...
Documented in [rust-patterns.md](../rust-patterns.md) §7 (joint space / hold-at). Right weighted bench tuning: `arm_3dof_right` — `position_slew_rad_s=0.08`, `max_lead=0.03`, velocity limits 2.0 rad/s, `fc=0.5` restored.
