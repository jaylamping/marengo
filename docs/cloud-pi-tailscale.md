...
| `invalid key: API key does not exist` | Wrong or stale key. Use a Tailscale **Auth key** (`tskey-auth-…`), not an API key. After updating the Cursor secret, **start a new Cloud Agent** (secrets inject only at boot) then `./scripts/setup-cloud-pi.sh --verify` |
