...
| `Could not resolve hostname marengo.local` in Docker deploy | mDNS does not resolve inside the container. Set `MARENGO_PI_HOST` to a reachable name (e.g. Tailscale `joey-robot.tail….ts.net` or Pi IP) — same override as MCP (`export` / `run-mcp.sh`, not `.cursor/mcp.json` env). |
