...
| `CAN send failed: No buffer space available` / ENOBUFS (`qlen=10`) | Kernel TX queue too short for mcp251x + multi-motor MIT. `scripts/can-up.sh` sets `txqueuelen` to **1000** (`MARENGO_CAN_TXQUEUELEN` to override). Re-run `marengo-can` / `pi_can_up`, then restart `marengo-pi` (bring-up downs the iface). Verify: `ip -details link show can0` shows `qlen 1000`. |
