...
| **Coupled gravity compensation** | Limb-level GravityComp acceptance after per-joint checks: all online/mapped joints in the limb in `GravityComp`; three-band static poses (arm-down, mid-range, elevated) with per-joint `|τ_meas − τ_g| < 0.20 Nm`, limb RMS residual `< 5%` of peak `|τ_g|`, and light dwell (≈5–10 s, drift `< 0.05 rad`); plus slow hand-guided float with feel + residual spot-checks. Not a separate control mode. _Avoid_: treating single-joint pitch T1 alone as limb G-comp done. |
| **Commissioning velocity baseline** | Per-joint Position-ladder speed SoT derived from the actuator manual, adjusted for bench bus voltage (e.g. 24 V vs 48 V rating) and a small additional derate, then written into config before the 25/50/75/100% ladder. Not assumed equal to whatever cruise/caps happen to be in YAML today. |
