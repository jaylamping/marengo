...
| **Commissioning scope** | Operator-selected limb(s)/joint set on Hardware that Enable is allowed to command when Robot Ready is false. Requires confirm; never includes Fault or OutOfLimits joints. Status still shows true Robot Ready separately. _Avoid_: silent JOINT_SUBSET-only Enable without UI attestation. |
...
| **Telemetry page** | Consul live read-only observation (ex-Subsystems). No Set Limits / Set Zero / membership SoT. Route `/telemetry`; `/subsystems` redirects. _Avoid_: Inventory table; Subsystems as nav label. |
