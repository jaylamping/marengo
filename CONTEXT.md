...
| **Hardware page** | Consul page for durable hardware-description SoT (master URDF + master YAML folder). Table-first joint list with optional 3D picker; unified settings sheet; Set Limits only here. Warn-only completeness — never hard-blocks the operator. |
| **Master URDF** | Sole live URDF SoT: `/opt/marengo/assets/urdf/marengo.urdf` (repo `assets/urdf/marengo.urdf`). Contributors land in `staging/` then `archive/<upload-id>/`. Match/merge by actuated joint **name**, not filename. |
| **Master YAML folder** | Sole live config SoT: `/opt/marengo/config/` (`robot` / `motors` / `control` / `homing`). Bringup-profile folders are not SoT. |
| **Accept = Active** | Resolve-wizard Accept promotes the resolved field set into active memory and write-behind durable files (ADR 0012). |
| **Limb subset (ephemeral)** | Caller-chosen joint subset at runtime (Testing / MCP harness) against the master URDF/YAML — not an alternate URDF or bringup tree. |
