...
| **Master URDF** | The combined hardware-description URDF the Hardware page treats as the durable assembly SoT — composition of limb/DoF contributions (target `marengo.urdf`). Limb/DoF uploads merge into it by actuator identity; Accept hot-reloads this working set. Limb-subset bringup for Testing/CLI is separate from this SoT. Unpowered limbs may remain in master for Consul visibility. |
| **Hardware (Consul)** | Consul page for durable robot description. Primary chrome is an interactive 3D humanoid (click joint → settings sheet); owns gospel mapped fields. Not live telemetry — that remains **Subsystems**. Route e.g. `/hardware`. |
| **Master YAML set** | Single on-Pi folder of durable `motors.yaml` / `control.yaml` / `homing.yaml` (etc.) keyed by actuator names alongside the master URDF. Replaces bringup-profile config directories as SoT. |
| **Bringup profile** | _(retired as SoT)_ Former `config/bringup/<slice>/` folders. Hardware/master URDF + master YAML set supersede them; contributor URDFs may be archived/versioned for history only. |
