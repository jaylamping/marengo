# Incoming vendor CAD

Drop vendor downloads here before promoting to `cad/vendor/`.

| File (you provide) | Registry `asset_id` |
|--------------------|---------------------|
| `grabcad_2020_aluminium_extrusions.SLDPRT` | `vendor_2020_black_extrusion` (archive; canonical: `../vendor_2020_black_extrusion.SLDPRT`) |
| `robstride_rs03_vendor.stp` | `actuator_rs03` (staged → `../vendor_robstride_rs03_vendor.*`) |
| `2020_corner_bracket.SLDPRT` | `bracket_2028_corner` (archive; canonical: `../vendor_2028_corner_bracket_vendor.SLDPRT`, promote via `npm run promote:corner-bracket` in solidworks-mcp) |
