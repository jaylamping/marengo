...
**Rev-a shoulder decision:** shoulder roll RS03 units live **inside** the 2020 cage (not on outer Y faces). Joint axes and names are unchanged; only packaging and brackets change.

| Instance | Joint | Axis | Mount region |
|----------|-------|------|----------------|
| `actuator_rs03_waist_yaw` | `waist_yaw` | **+Z** | Bottom **40 mm** waist bay (`waist_bay_top`) |
| `actuator_rs03_left_shoulder_roll` | `left_shoulder_roll` | **+X** | **Inside** cage, **+Y** half, at `shoulder_plane` |
| `actuator_rs03_right_shoulder_roll` | `right_shoulder_roll` | **+X** | **Inside** cage, **−Y** half, at `shoulder_plane` |
...
**Kinematics unchanged:** `left/right_shoulder_roll` remain **+X**; URDF still torso → roll link.
...
**Required orientation at shoulder:** roll axis **+X**; pack body so **99 mm spans Y** and **57 mm spans X** (not 99 mm into the 85 mm depth). Verify in a test assembly before printing brackets.
...
| `shaft_axis` | Motor output (+X) | Motor output (+Z) |
| `cable_exit` | Toward chest center (down/back) | Up into torso |

Layout part anchors to add:

- `shoulder_mount_left` / `shoulder_mount_right` — coord sys at shoulder plane, motor center, +X axis
...
2. **Motor:** RS03 flange; output shaft **+X** toward arm (arm exits shell via cutout, not motor poke).
...
- [ ] `marengo_kinematics_consistency` still finds `waist_yaw`, `left_shoulder_roll`, `right_shoulder_roll`
...
2. **Shoulder pitch RS03** (future) — also likely inside or stacked above roll motor; plan **two-high** shoulder stack in Y-half if pitch stays RS03.
