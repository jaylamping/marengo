...
function isExplicitUnixShell(cmd) {
    return (/^\s*(bash|sh|dash|zsh|wsl|wsl\.exe)\b/i.test(cmd) ||
...
function bashismIssues(cmd) {
    const issues = [];
    if (/&&|\|\|/.test(cmd)) {
        issues.push('bash &&/|| chaining - use "; if ($LASTEXITCODE -eq 0) { ... }"');
    }
    if (/<<\s*['"]?[\w-]/.test(cmd)) {
        issues.push("bash heredoc - pipe a PowerShell here-string into sh, or run via bash -c");
    }
    ...
    if (/\$\(\s*(cat|dirname|basename|pwd|which|command)\b/.test(cmd)) {
        issues.push("bash $(cmd) - use PowerShell equivalents (Get-Content, Split-Path, Get-Command)");
    }
