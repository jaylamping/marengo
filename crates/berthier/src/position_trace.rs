...
impl PositionTrace {
    ...
    fn open(path: &Path, loop_hz: u32) -> std::io::Result<Self> {
        ...
        if is_new {
            writeln!(
                writer,
                "tick,t_ms,joint,q,dq,dq_traj,q_des,target,lead,lead_sat,tracking_error,settle_error,phase,kp,kd,tau_g,tau_f,tau_d,tau_ff_cmd,tau_meas,tau_err,kp_mit,kd_mit"
            )?;
        }
...
impl PositionTraceRow<'_> {
    pub fn format_csv_with_meta(&self, tick: u64, t_ms: u64) -> String {
        ...
        format!(
            "{tick},{t_ms},{joint},{q:.6},{dq:.6},{dq_traj:.6},{q_des:.6},{target:.6},{lead:.6},{lead_sat},{tracking_error:.6},{settle_error:.6},{phase},{kp:.3},{kd:.3},{tau_g:.6},{tau_f:.6},{tau_d:.6},{tau_ff_cmd:.6},{tau_meas:.6},{tau_err:.6},{kp_mit:.3},{kd_mit:.3}",
            tick = tick,
            t_ms = t_ms,
